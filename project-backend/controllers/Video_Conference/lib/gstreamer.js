...
module.exports = class GStreamer {
	...
	_createProcess() {
		...
		// console.log('Gstreamer before process');
		this._process = child_process.spawn(exe, this._commandArgs, {
			detached: true,
			shell: true
		});

		console.log(`Inside gstreamer after spawn, pid:${this._process.pid}`);
		...
		//   console.log(`gstreamer::process::stdout::data [data: ${data}]`)
		// );
	}
	...
	convertToMp4() {
		setTimeout(() => {
			...
			ffmpeg.once('close', () => {
				fs.unlink(`${RECORD_FILE_LOCATION_PATH}/${this._rtpParameters.fileName}.webm`, (err) => {
					if (err)
						console.log("Unable to delete webm file");
				})
			});
		}, 2000);
	};
	...
	get _rtcpArgs() {
		...
		// console.log(video.remoteRtcpPort);
		// console.log(video.remoteRtpPort);
		return [
