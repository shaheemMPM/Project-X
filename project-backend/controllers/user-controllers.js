...
const signup = async (req, res, next) => {
  ...
  try {
    ...
    console.error(
      `Error while checking for existing user in sign up 1 \n${err}`
    );
    return next(
  ...
  if (existingUser) {
    ...
    return next(
      new HttpError("username exists already, please login instead.", 422)
    );
  }
  ...
  try {
    ...
    console.error(
      `Error while checking for existing user in sign up 2 \n${err}`
    );
    return next(
      new HttpError("Signing up failed, please try again later.", 500)
    );
  ...
  if (existingUser) {
    ...
    return next(
      new HttpError("email exists already, please login instead.", 422)
    );
  }
